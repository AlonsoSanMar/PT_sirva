...
let ueasActuales = document.getElementById('ueasActuales').value;
let ueasActualesArr = ueasActuales.split(',').map(function(item) {
    return item.trim(); // Eliminar espacios en blanco alrededor de cada elemento
});

async function cargarUeasActuales(ueasActualesArr){
    const ueasActualesArrString = ueasActualesArr.join(',');
    const url = `/registro/buscarUeasActuales/${ueasActualesArrString}`;
    const response = await fetch(url);
    const data = await response.json();
    for(let i = 0; i < data.length; ++i) {
        agregaUeaActual(data[i]);
    }
}

cargarUeasActuales(ueasActualesArr);
...
function agregaUeaActual(uea) {
    ...
    if (ueas.find(u => u.clave === uea.clave) === undefined) {
        ...
        let btnAprobado = document.createElement('button'); //Usando font awesome e 'i'
        btnAprobado.textContent = "Aprobado";
        btnAprobado.classList.add('btn-aprobado');
        btnAprobado.addEventListener('click', () => {
            contenedor.removeChild(elemento); //Elimino visualmente
            sumaCreditos += parseInt(uea.creditos);
            ueas = ueas.filter(u => !u.nombre.toLowerCase().includes(uea.nombre.toLowerCase()))
            //eliminaUeaDeSeleccion(uea);
            actualizarFormulario();
            comparaCreditosMinimos();
        ...
        contBtns.classList.add('cont-btns');
        contBtns.appendChild(btnEliminar);
        contBtns.appendChild(btnAprobado);
        ...
        elementoHijo.appendChild(contBtns);
...
function recogerDatos() {
    ...
    let creditos_actuales = parseInt(document.getElementsByName('creditos_actuales')[0].value) + sumaCreditos;
    ...
    registroUsuario(matricula, creditos_actuales, ueasSeleccionadas);
}

//Checar bien las respuestas

function registroUsuario(matricula, creditos_actuales, ueasSeleccionadas) {
    const data = {
        matricula: matricula,
